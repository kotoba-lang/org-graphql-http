(ns kotobase.protocols.graphql.json
  ...
  this one zero-dependency file). Exists so the GraphQL-over-HTTP endpoint
  (JSON request/response, per graphql.github.io/graphql-over-http/) has no
  host-JSON dependency: the same handler runs under nbb/cljs (first-class
