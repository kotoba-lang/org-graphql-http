...
;; at the SHA pinned in deps.edn (graphql, kotobase-query) or transitively
;; in kotobase-query's / arrangement's own deps.edn (kotobase, arrangement,
...
(ns run-tests
  (:require [cljs.test :as t]
            [kotobase.protocols.graphql-test]))

(defmethod t/report [:cljs.test/default :end-run-tests] [m]
  (when-not (t/successful? m)
    (set! (.-exitCode js/process) 1)))

(t/run-tests 'kotobase.protocols.graphql-test)
